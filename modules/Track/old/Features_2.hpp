...
public:
   ...
   virtual ~Features() {/* dtor */}

   virtual void Detect(const Mat& image, vector<cv::KeyPoint>& keys) const;
   virtual void Extract(const Mat& image, vector<cv::KeyPoint>& keys, Mat& descriptors) const;
   virtual void Match(const Mat& descTarget, const Mat& descScene, vector<Point2f>& target, vector<Point2f> scene) const;

   virtual void OptimizeFor(OptFlag flag);
